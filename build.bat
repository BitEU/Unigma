@echo off
REM Build script for Unigma: The Little UNIVAC Enigma Simulator
REM Unified script for MSVC and MinGW compilation
REM Supports Windows and UNIVAC cross-compilation

echo.
echo ========================================
echo   Unigma - Unified Build Script
echo   The Little UNIVAC Enigma Simulator
echo ========================================
echo.
...
:BUILD_START
echo.
echo Building Unigma: The Little UNIVAC Enigma Simulator...
echo.
...
if exist unigma_univac.exe del /Q unigma_univac.exe
if exist unigma_univac.o del /Q unigma_univac.o
if exist *.o del /Q *.o
echo.
...
echo Compiling unigma.c...
gcc -c -DUNIVAC -O2 -Wall unigma.c -o unigma_univac.o

if %ERRORLEVEL% NEQ 0 (
    echo ERROR: Failed to compile unigma.c
    pause
    exit /b 1
)

echo Linking...
gcc -o unigma_univac.exe unigma_univac.o
...
REM Clean previous build artifacts
echo Cleaning previous build artifacts...
if exist unigma_mingw.exe del /Q unigma_mingw.exe
if exist unigma.obj del /Q unigma.obj
if exist *.o del /Q *.o
echo.
...
if %ERRORLEVEL% EQU 0 (
    ...
    echo ========================================
    echo.
    echo Output: unigma_mingw.exe

    REM Display file size
    for %%A in (unigma_mingw.exe) do (
        echo File size: %%~zA bytes
    )
    echo.
    echo Optimization level: MAXIMUM (-O3 + LTO + native CPU^)
    echo.
    echo To run Unigma, type: unigma_mingw.exe
    echo.
) else (
    echo.
    echo ========================================
...
cl /W4 /O2 /Fe:unigma.exe unigma.c
...
if %ERRORLEVEL% EQU 0 (
    ...
    echo ========================================
    echo.
    echo Output: unigma.exe
    echo.
    echo To run: unigma.exe
) else (
    echo.
    echo ========================================
    echo   BUILD FAILED
